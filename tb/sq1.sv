...
class er_write_read_sequence extends base_sequence;
        `uvm_object_utils(er_write_read_sequence)

        function new(string name = "er_write_read_sequence");
                super.new(name);
        endfunction

        virtual task body();
                bit[31:0] read_data;
                `uvm_info(get_type_name(), "Starting error write and read sequence", UVM_LOW)
                write_reg(32'd12, 32'hAA);
                #100ns;
                read_reg(32'd12, read_data);
