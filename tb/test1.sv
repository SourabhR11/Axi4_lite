...
// TEST-027: Error Write read test
class er_write_read_test extends base_test;

        `uvm_component_utils(er_write_read_test)

        function new(string name = "er_write_read_test", uvm_component parent = null);
        super.new(name, parent);
endfunction

virtual task run_phase(uvm_phase phase);

        er_write_read_sequence seq;

        phase.raise_objection(this);

        apply_reset();

        seq = er_write_read_sequence::type_id::create("seq");
