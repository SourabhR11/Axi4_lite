...
class unaligned_addr_test extends base_test;
  ...
  virtual task run_phase(uvm_phase phase);
    unaligned_addr_sequence seq;
    phase.raise_objection(this);

    apply_reset();

    seq = unaligned_addr_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);
...
class invalid_addr_test extends base_test;
  ...
  virtual task run_phase(uvm_phase phase);
    invalid_addr_sequence seq;
    phase.raise_objection(this);

    apply_reset();

    seq = invalid_addr_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);
...
// LED IRQ Behavior
class led_irq_test extends base_test;
  `uvm_component_utils(led_irq_test)

  function new(string name = "led_irq_test", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  virtual task run_phase(uvm_phase phase);
    led_irq_sequence seq;
    phase.raise_objection(this);

    apply_reset();

    seq = led_irq_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);
...
// Only OKAY Response Test
class only_OKAY_resp_test extends base_test;

  `uvm_component_utils(only_OKAY_resp_test)

  function new(string name = "only_OKAY_resp_test", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  virtual task run_phase(uvm_phase phase);
    only_OKAY_resp_sequence seq;

    phase.raise_objection(this);

    // Apply reset before starting sequence
    apply_reset();

    // Create and start the sequence
    seq = only_OKAY_resp_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);

    // Allow simulation time for transactions
    #5000ns;

    phase.drop_objection(this);
  endtask

endclass : only_OKAY_resp_test

// Address Out-of-Range Test
class addr_out_of_range_test extends base_test;

  `uvm_component_utils(addr_out_of_range_test)

  function new(string name = "addr_out_of_range_test", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  virtual task run_phase(uvm_phase phase);
    addr_out_of_range_sequence seq;
    ...
    // Create and start the sequence
    seq = addr_out_of_range_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);
...
endclass : addr_out_of_range_test

// outstanding transactions Test
class outstanding_test extends base_test;

  `uvm_component_utils(outstanding_test)

  function new(string name = "outstanding_test", uvm_component parent = null);
    super.new(name, parent);
  endfunction

  virtual task run_phase(uvm_phase phase);
    outstanding_sequence seq;
    ...
    // Create and start the sequence
    seq = outstanding_sequence::type_id::create("seq");
    seq.start(tb_env.axi_agt.sequencer);
